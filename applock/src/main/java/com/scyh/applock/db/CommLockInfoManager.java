...
import com.scyh.applock.model.FaviterInfo;
import com.scyh.applock.utils.DataUtil;
...
public class CommLockInfoManager {
	 ...
	 * 将手机应用信息插入数据库
	 */
	public synchronized void instanceCommLockInfoTable(List<ResolveInfo> resolveInfos)
		...
		for (ResolveInfo resolveInfo : resolveInfos) {
			...
			ApplicationInfo appInfo = mPackageManager.getApplicationInfo(commLockInfo.getPackageName(),
	...
	public int getLockedCount() {
		List<CommLockInfo> lockInfos = where("isLocked=1").find(CommLockInfo.class);
		if (lockInfos != null) {
	...
	private Comparator commLockInfoComparator = new Comparator() {
		...
		public int compare(Object lhs, Object rhs) {
			...
			if (leftCommLockInfo.isFaviterApp() && !leftCommLockInfo.isLocked() && !rightCommLockInfo.isFaviterApp()
					&& !rightCommLockInfo.isLocked()) {
				return -1;
			} else if (leftCommLockInfo.isFaviterApp() && leftCommLockInfo.isLocked()
					&& !rightCommLockInfo.isFaviterApp() && !rightCommLockInfo.isLocked()) {
				return -1;
			} else if (leftCommLockInfo.isFaviterApp() && leftCommLockInfo.isLocked()
					&& rightCommLockInfo.isFaviterApp() && !rightCommLockInfo.isLocked()) {
				if (leftCommLockInfo.getAppInfo() != null && rightCommLockInfo.getAppInfo() != null)
					return 1;
				else
					return 0;
			} else if (!leftCommLockInfo.isFaviterApp() && leftCommLockInfo.isLocked()
